...
type Run struct {
	...
	SHA256  string
	Module  string
	// Workspace maps the repository-relative directory of each go.work module
	// outside the repository root to its module path.
	Workspace map[string]string
}

// profileKey builds the profile file name of a repository path from the
// innermost module containing it, or returns "" when no module contains it.
func (r Run) profileKey(path string) string {
	module, rel, depth := r.Module, path, 0
	if module == "" {
		depth = -1
	}
	for dir, m := range r.Workspace {
		if strings.HasPrefix(path, dir+"/") && len(dir) > depth {
			module, rel, depth = m, path[len(dir)+1:], len(dir)
		}
	}
	if depth < 0 {
		return ""
...
func ModulePath(dir string) (string, error) {
	b, err := readBounded(filepath.Join(dir, "go.mod"))
	if err != nil {
...
// Modules reads the root module path and the go.work modules of a snapshot, so
// a repository whose Go module lives in a subdirectory can still be measured.
// The root module path is empty when only go.work declares modules. Every
// declared module must resolve unambiguously; a workspace entry that escapes
// the snapshot or has no readable module path fails the whole lookup.
func Modules(dir string) (string, map[string]string, error) {
	root, rootErr := ModulePath(dir)
	b, err := readBounded(filepath.Join(dir, "go.work"))
	if err != nil {
		if os.IsNotExist(err) {
			return root, nil, rootErr
		}
		return "", nil, ErrModulePath
	}
	uses, ok := workspaceUses(string(b))
	if !ok {
		return "", nil, ErrModulePath
	}
	workspace := map[string]string{}
	for _, use := range uses {
		module, err := ModulePath(filepath.Join(dir, filepath.FromSlash(use)))
		if err != nil {
			return "", nil, ErrModulePath
		}
		if use == "." {
			root, rootErr = module, nil
			continue
		}
		workspace[use] = module
	}
	if rootErr != nil {
		root = ""
	}
	if root == "" && len(workspace) == 0 {
		return "", nil, ErrModulePath
	}
	return root, workspace, nil
}

// workspaceUses returns the cleaned, snapshot-relative directories of the use
// directives of a go.work file, in both the single-line and block forms.
func workspaceUses(body string) ([]string, bool) {
	var uses []string
	block := false
	for _, line := range strings.Split(body, "\n") {
		...
		use := strings.Trim(fields[0], `"`)
		clean := path.Clean(use)
		if use == "" || strings.HasPrefix(use, "/") || strings.ContainsAny(use, ":\\\x00") || clean == ".." || strings.HasPrefix(clean, "../") {
			return nil, false
		}
		uses = append(uses, clean)
	}
	return uses, !block
}
