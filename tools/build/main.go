...
// Run from the repository root: go run ./tools/build
package main
...
func run(ctx context.Context) error {
	...
	for _, path := range []string{"go.mod", "cmd/swiftproof/main.go", "LICENSE", "README.md"} {
		if _, err := os.Stat(path); err != nil {
			return fmt.Errorf("run from the SwiftProof repository root: %w", err)
		}
	}
...
func build(ctx context.Context, goBinary, out, version, osName, arch string) (string, error) {
	...
	files := []entry{{destination, name, 0755}, {"LICENSE", "LICENSE", 0644}, {"README.md", "README.md", 0644}}
